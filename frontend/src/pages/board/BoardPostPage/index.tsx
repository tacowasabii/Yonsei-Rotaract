    ...
    <PageLayout>
      ...
      {post && !user && post.visibility === "members" && (
        <div className="bg-white rounded-2xl shadow-card overflow-hidden">
          <div className="px-8 pt-8 pb-6 border-b border-surface-container">
            ...
            <p className="text-sm text-on-surface-variant">{formatDate(post.created_at)}</p>
          </div>
          <div className="px-8 py-16 flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <p className="font-bold text-on-surface text-lg mb-1">회원 전용 게시글입니다</p>
              <p className="text-sm text-on-surface-variant">로그인 후 내용을 확인할 수 있습니다.</p>
            </div>
            <button
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl shadow-card overflow-hidden">
            <div className="px-8 py-5 border-b border-surface-container">
              <h2 className="font-bold text-on-surface">댓글 ({comments.length})</h2>
            </div>
            ...
            <div className="divide-y divide-surface-container">
              ...
              {!isCommentsLoading && comments.length === 0 && (
                <div className="px-8 py-8 text-center text-sm text-on-surface-variant">
                  아직 댓글이 없습니다.
                </div>
              )}
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            {isCommentAuthor && (
                              <button
                                ...
                                className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold text-on-surface-variant hover:bg-surface-container transition-all"
                              >
                                <span className="material-symbols-outlined text-sm">edit</span>
                                수정
                              </button>
                            ...
                            {canDelete && (
                              <button
                                ...
                                className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold text-red-400 hover:bg-red-50 transition-all"
                              >
                                <span className="material-symbols-outlined text-sm">delete</span>
                                삭제
                              </button>
            ...
            <div className="px-8 py-5 bg-surface-container-low border-t border-surface-container">
              {user ? (
                ...
                <div className="flex items-center justify-between gap-4">
                  <p className="text-sm text-on-surface-variant">댓글을 작성하려면 로그인이 필요합니다.</p>
                  <button
                    onClick={() => navigate("/login")}
      ...
      {deletingCommentId && (
        <DeleteConfirmModal
          ...
          onConfirm={() => deleteComment(deletingCommentId, { onSuccess: () => setDeletingCommentId(null) })}
