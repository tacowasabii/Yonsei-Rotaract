...
export default function MessageRow({
  ...
  const counterpartProfile =
    box === "received" ? message.sender : message.recipient;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span
              className={`text-sm ${isUnread ? "font-black text-on-surface" : "font-semibold text-on-surface"}`}
            >
              {counterpart}
            </span>
          ...
          <p
            className={`text-sm truncate mt-0.5 ${isUnread ? "font-bold text-on-surface" : "text-on-surface-variant"}`}
          >
            {message.title}
          </p>
        </div>
